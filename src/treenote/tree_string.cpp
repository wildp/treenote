...
namespace treenote
{
    ...
    namespace detail
    {
        namespace
        {
            ...
            inline tree_string::table_entry& get_entry(tree_string::table_t& pt, std::size_t line, std::size_t entry_index)
            {
            ...
            tree_string::opt_idx_pair entry_index_within_table_line(const tree_string::table_line& line, std::size_t pos)
            {
            ...
            inline pt_cmd::delete_entry::merge_info make_merge_info(tree_string::table_t& pt, std::size_t line, std::size_t entry_index)
            {
            ...
            inline bool entry_has_no_mb_char(const tree_string::table_entry& entry)
            {
                return entry.display_length == entry.byte_length;
            }
            
            void grow_entry_rhs(tree_string::table_entry& entry, std::size_t display_amt, std::size_t byte_amt)
            {
                entry.display_length += display_amt;
                entry.byte_length += byte_amt;
            }
            
            void shrink_entry_rhs(tree_string::table_entry& entry, std::size_t display_amt, std::size_t byte_amt)
            {
            ...
            void shrink_entry_lhs(tree_string::table_entry& entry, std::size_t display_amt, std::size_t byte_amt)
            {
                /* assume: display_amt <= entry.display_length && byte_amt <= entry.byte_length */
            ...
            void unshrink_entry_lhs(tree_string::table_entry& entry, std::size_t display_amt, std::size_t byte_amt)
            {
            ...
            void insert_entry_naive(tree_string::table_t& pt, std::size_t line, std::size_t entry_index, const tree_string::table_entry& entry)
            {
                auto& table_line{ pt.at(line) };
                table_line.insert(std::ranges::begin(table_line) + static_cast<ptrdiff_t>(entry_index), entry);
            }
            
            void delete_entry_and_merge(tree_string::table_t& pt, std::size_t line, std::size_t entry_index)
            {
                auto& table_line{ pt.at(line) };
                ...
                auto erase_pos{ std::ranges::begin(table_line) + static_cast<ptrdiff_t>(entry_index) };
            ...
            void split_entry_remove_inside(tree_string::table_t& pt, const std::string& buffer, std::size_t line, std::size_t original_entry_index, std::size_t l_boundary_pos, std::size_t r_boundary_pos)
            {
                ...
                const tree_string::table_entry right{ original.start_index + skipped_bytes, original.display_length - r_boundary_pos, original.byte_length - skipped_bytes };
                original.display_length = l_boundary_pos;
                original.byte_length = left_bytes;
                
                auto insert_pos{ std::ranges::begin(table_line) + static_cast<ptrdiff_t>(original_entry_index) + 1 };
                table_line.insert(insert_pos, right);
            }
            
            void undo_split_entry_remove_inside(tree_string::table_t& pt, std::size_t line, std::size_t original_entry_index, std::size_t r_boundary_pos)
            {
            ...
            void split_entry_and_insert(tree_string::table_t& pt, const std::string& buffer, std::size_t line, std::size_t original_entry_index, std::size_t pos_in_entry, const tree_string::table_entry& entry)
            {
                ...
                std::vector<tree_string::table_entry> to_insert{ entry };
            ...
            void undo_split_entry_and_insert(tree_string::table_t& pt, std::size_t line, std::size_t original_entry_index)
            {
...
//                table_line.erase(erase_begin, erase_end);
            ...
            void undo_delete_entry_and_merge(tree_string::table_t& pt, const std::string& buffer, std::size_t line, std::size_t idx, const tree_string::table_entry& entry, const pt_cmd::delete_entry::merge_info& merge_pos)
            {
            ...
            void split_lines(tree_string::table_t& pt, const std::string& buffer, std::size_t line, std::size_t pos)
            {
            ...
            void join_lines(tree_string::table_t& pt, std::size_t line_after)
            {
                ...
                if (!snd.empty())
                {
                    if (!fst.empty())
                    {
                        if (fst.back().start_index + fst.back().byte_length == snd.front().start_index)
                        {
                            ...
                            if (snd.size() > 1)
                            {
                                auto snd_begin{ std::ranges::begin(snd) + 1 };
                                auto snd_end{ std::ranges::end(snd) };
                                fst.reserve(fst.size() + std::ranges::distance(snd_begin, snd_end));
                                std::move(snd_begin, snd_end, std::back_inserter(fst));
                            }
                        }
                        else
                        {
                            auto snd_begin{ std::ranges::begin(snd) };
                            auto snd_end{ std::ranges::end(snd) };
                            fst.reserve(fst.size() + std::ranges::distance(snd_begin, snd_end));
                            std::move(snd_begin, snd_end, std::back_inserter(fst));
                        }
    ...
    tree_string::tree_string(std::string_view input_sv) :
            buffer_{ input_sv }, buffer_len_{ utf8::length(buffer_).value_or(0) }
    {
        ...
        {
            piece_table_vec_.back().push_back(table_entry{ 0, buffer_len_, buffer_.size() });
        }
    }
    
    tree_string::tree_string(std::pair<std::string, std::size_t>&& input) :
    ...
    void tree_string::add_line(std::pair<std::string, std::size_t>&& more_input)
    {
        piece_table_vec_.emplace_back();
    ...
    tree_string tree_string::make_copy() const
    {
        ...
        {
            tree_string result{ { to_str(0), line_length(0) } };
    ...
    bool tree_string::insert_str(std::size_t line, std::size_t pos, const std::string& str, std::size_t& cursor_inc_amt)
    {
        ...
        else if (pos == 0)
        {
            ...
            exec(table_command{ pt_cmd::insert_entry{ line, 0, table_entry{ buffer_begin_pos, utf8len, str.size() } } });
        ...
        else
        {
            ...
            for (std::size_t i{ 0 }, accumulated_len{ 0 }; i < table_line.size(); ++i)
            {
                if (pos < accumulated_len + table_line[i].display_length)
                {
                    ...
                    exec(table_command{ pt_cmd::split_insert{ line, i, pos - accumulated_len, table_entry{ buffer_begin_pos, utf8len, str.size() } } });
                ...
                else if (pos == accumulated_len + table_line[i].display_length || i + 1 == table_line.size())
                {
                    ...
                    else
                    {
                        ...
                        exec(table_command{ pt_cmd::insert_entry{ line, i + 1, table_entry{ buffer_begin_pos, utf8len, str.size() } } });
                        last_inserted_te_idx_ = { line , i + 1 };
                    }
                    break;
                }
    ...
    bool tree_string::delete_char_before(std::size_t line, std::size_t pos, std::size_t& cursor_dec_amt)
    {
        ...
        if (last_action_ == pt_cmd_type::deletion_b && last_interacted_pos_ == std::pair{ line, pos } && !piece_table_hist_.empty() && pos > 0)
        {
            ...
            if (eiwtl.has_value())
            {
                ...
                std::reference_wrapper<table_command> last_sub_cmd{ piece_table_hist_.back() };
                ...
                if (std::holds_alternative<pt_cmd::split_delete>(last_sub_cmd.get()))
                {
                    if (table_line[entry_idx].display_length == 1)
                    {
                        if (entry_idx + 1 < table_line.size())
                        {
                            ...
                            const table_entry before_copy{ table_line[entry_idx + 1] };
                            invoke_reverse(last_sub_cmd.get());
                            const table_entry after_copy{ table_line[entry_idx] };
                    ...
                    else
                    {
                        ...
                        if (!detail::entry_has_no_mb_char(entry))
                         ...
                         std::holds_alternative<pt_cmd::delete_entry>(last_sub_cmd.get()))
                {
                    ...
                    else if (pos_in_entry == 0)
                    {
                        ...
                        if (!detail::entry_has_no_mb_char(table_line[entry_idx]))
        ...
        for (std::size_t i{ 0 }, accumulated_len{ 0 }; !new_command_issued && i < table_line.size(); ++i)
        {
            if (table_line[i].display_length > 0)
            {
                if (pos == accumulated_len + table_line[i].display_length)
                {
                    ...
                    {
                        /* delete last char from pt entry by shrinking rhs of entry */
                        if (detail::entry_has_no_mb_char(table_line[i]))
                ...
                if (std::holds_alternative<pt_cmd::split_delete>(last_sub_cmd.get()))
                {
                    if (table_line[entry_idx].display_length == 1)
                    {
                        if (entry_idx > 0)
                        {
                            ...
                            invoke_reverse(last_sub_cmd.get());
                            const table_entry after_copy{ table_line[entry_idx - 1] };
            ...
            for (const auto& pte: piece_table_line)
            {
                if (ignored_count >= pos)
                {
                    ...
                    else
                    {
                        ...
                        if (detail::entry_has_no_mb_char(pte))
                        {
    ...
    std::size_t tree_string::line_length(std::size_t line) const
    {
        if (line < piece_table_vec_.size())
            return std::transform_reduce(std::ranges::cbegin(piece_table_vec_[line]), std::ranges::cend(piece_table_vec_[line]),
                                         0uz, std::plus{}, [](const table_entry& te){ return te.display_length; });
        else
            return 0;
    }
    ...
    cmd_names tree_string::get_current_cmd_name() const
    {
        ...
        std::reference_wrapper<const table_command> cmd{ piece_table_hist_[piece_table_hist_pos_ - 1] };
    ...
    void tree_string::clear_hist_if_needed()
    {
        ...
        else if (piece_table_hist_pos_ == piece_table_hist_.size())
        {
            ...
            if (piece_table_hist_pos_ == detail::max_hist_size_)
            {
                ...
                auto hist_begin{ std::ranges::begin(piece_table_hist_) + (static_cast<std::ptrdiff_t>(piece_table_hist_pos_) / 2) };
                auto hist_end{ std::ranges::end(piece_table_hist_) };
    ...
    std::string tree_string::index_of_char_within_entry(const table_entry& entry, std::size_t pos_in_entry) const
    {
